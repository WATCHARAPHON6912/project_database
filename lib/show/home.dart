...
class _homeState extends State<home> {
  ...
  void initState() {
    ...
    data = [];
    DB_read("SELECT student_id,name,email,phone FROM new WHERE email = '${widget.email}'")
        .then((value) {
      data = value;
    });
    print("Home");
  ...
  String list_show(index){
    String text = "";
    if (index==0)text = "รหัสนักศึกษา "+data[index]; 
    else if (index==1)text = "ชื่อ "+data[index]; 
    else if (index==2)text = "อีเมล "+data[index]; 
